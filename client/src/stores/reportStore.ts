...
export const useReportStore = create<ReportState>((set, get) => ({
  ...
  fetchMyReports: async (hubDTag, hubCreatorPubkey, hubSecretHex, myPubkey, relays) => {
  ...
  submitReport: async ({
    ...
    // PoW + sign (with automatic retry if signer invalidates PoW)
    const signed = await mineAndSign(unsigned, minPow || 0, pubkey, signer, privateKey)
    await publishToSpecificRelays(getPublishRelays(relays), signed)

    // Optimistic local update — add to myReportsByHub
