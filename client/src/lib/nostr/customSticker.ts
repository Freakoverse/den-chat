...
/** One batch of an author's sticker-set events (client + NIP-65 relays), deduped by d-tag (latest wins). */
async function fetchStickerSetEventsByAuthor(pubkey: string, opts?: { limit?: number; until?: number }): Promise<Event[]> {
  const filter = {
    kinds: [KIND_STICKER_SET],
    authors: [pubkey],
...
export async function fetchStickerSetsByAuthor(pubkey: string): Promise<StickerSet[]> {
  const sets: StickerSet[] = []
  for (const ev of await fetchStickerSetEventsByAuthor(pubkey)) {
    const parsed = parseStickerSetEventBroad(ev)
    if (parsed && parsed.stickers.length > 0) sets.push(parsed)
  }
  return sets
...
/** A page of the author's sets for cursor pagination — see AuthorSetBatch in customEmoji. */
export async function fetchStickerSetsByAuthorBatch(pubkey: string, opts: { limit: number; until?: number }): Promise<{
  items: { set: StickerSet; createdAt: number }[]
  oldest?: number
  rawCount: number
}> {
  const events = await fetchStickerSetEventsByAuthor(pubkey, opts)
  const items: { set: StickerSet; createdAt: number }[] = []
  let oldest: number | undefined
  for (const ev of events) {
    if (oldest === undefined || ev.created_at < oldest) oldest = ev.created_at
    const parsed = parseStickerSetEventBroad(ev)
    if (parsed && parsed.stickers.length > 0) items.push({ set: parsed, createdAt: ev.created_at })
  }
